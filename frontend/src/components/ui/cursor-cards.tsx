...
export function CursorCardsContainer({
  ...
  const enhancedChildren = React.Children.map(children, (child) => {
    if (React.isValidElement(child) && child.type === CursorCard) {
      return React.cloneElement(child as React.ReactElement<InternalCursorCardProps>, {
...
export function CursorCard({
  ...
  return (
    <div ref={elementRef} className={cn("group relative rounded-[inherit]", className)}>
      {/* Animated gradient border that tracks the cursor */}
      <motion.div
        className="pointer-events-none absolute inset-0 rounded-[inherit]"
        style={{ background: gradientBackground }}
      />
      {/* Card surface — sits 1px inside the gradient to reveal it as a border */}
      <div className="absolute inset-px rounded-[inherit] bg-card" />
      {/* Inner illumination that follows the cursor */}
      <motion.div
        className={cn(
