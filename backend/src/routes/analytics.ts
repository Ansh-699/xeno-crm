 ...
 *
 * Returns:
 *  - perChannel: per-channel aggregated stats (sent, delivered, opened, clicked, failed)
 *  - campaigns: per-campaign stats breakdown with channel info
 *  - overview: total sent, avg delivery rate, best channel, total campaigns
 */
router.get("/", async (_req: Request, res: Response) => {
  try {
      ...
      campaigns.map(async (camp) => {
        ...
        if (Object.keys(stats).length === 0) {
          ...
          stats = Object.fromEntries(
            dbStats.map((s) => [s.status, s._count])
          );
        }
    ...
    const totalFailed = campaignDetails.reduce(
      (s, c) => s + c.stats.failed,
      0
    );
